...
watch(
  () => props.modelValue,
  (value) => {
    form.value = value ? clonePlain(value) : emptySystem()
  },
  { immediate: true },
)
...
function updateColor(index: number, color: unknown) {
  form.value.palette[index] = String(color)
}
...
function save() {
  emit('save', clonePlain(form.value))
}
</script>
...
<template>
  <UModal
    ...
    title="Editor de sistema visual"
  ...
  >
    <template #body>
      ...
      >
        ...
        <UFormField
          class="full"
          label="Iconos de @nuxt/icon, separados por comas"
        >
          <UInput
            v-model="iconsText"
            required
        ...
        <UFormField
          class="full"
          label="Título de muestra"
        >
          <UInput
            v-model="form.sampleTitle"
            required
          />
        </UFormField>
        <UFormField
        ...
        <div class="full color-editor">
          ...
          <div
            v-for="(color, index) in form.palette"
            :key="index"
            class="color-row"
          >
            <input
              v-model="form.palette[index]"
              type="color"
            >
            <UInput
              :model-value="color"
